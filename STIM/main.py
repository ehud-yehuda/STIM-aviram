...
if constants.STRATEGY == constants.NSTEP_QL:
    if len(sys.argv) > 1:
        ...
        if constants.REWARD_TYPE == constants.IMMEDIATE_REWARD:
            ...
            REWARD_NAME = 'InNorm_' + str(constants.DIFUSION_WEIGHT) + '_' + str(constants.COUNT_WEIGHT) + '_' + str(
                constants.DEGREE_WEIGHT)
        else:
            REWARD_NAME = 'ImR'
        ...
        constants.SUMMARY_NAME = METHOD_NAME + '_' + Q_NAME + '_VAR_' + LAYER_NAME + '_' + TRAIN_NAME + '_NORM=' + \
                                 ...
                                 "{:.0e}".format(constants.LEARNING_RATE) + '_MinLR=' + "{:.0e}".format(
            constants.MIN_LEARNING_RT) + \
    ...
    with tf.Session(config=config) as session:
        ...
        summary = Summary(constants.SUMMARY_NAME, summary_writer, constants.SUMMARY_INT_SCR, constants.SUMMARY_INT_LOSS,
                          th_file)
        with tf.device("/cpu:0"):
            master_worker = NStep_QL_agent('global', n_steps, graph_count, session, constants.LEARNING_RATE,
                                           constants.GAMMA,
                                           constants.EXPLORATION_RT, constants.BATCH_SIZE, constants.PCT_MAX_STEPS,
                                           summary)
            workers = []
            for i in range(constants.N_THREADS):
                print(i)
                workers.append(
                    NStep_QL_agent(i, n_steps, graph_count, session, constants.LEARNING_RATE, constants.GAMMA,
